	...
	for ( var i = 0; i < options.startTime.length; i++) {
		if (parseInt(options.startTime[i]) >= 0) {
			...
			if (options.continuous === true) {
				...
				// a max of 5. Conditional max is used when the left digit has
				// reach
				// the max. For example second "hours" digit has a conditional
				// max of 4
...
var makeMovement = function(elem, steps, isForward, options) {
	...
	digits[elem].current = digits[elem].current
			+ ((isForward) ? steps : -steps);
...
jQuery.fn.countdown = function(userOptions) {
	...
	if (userOptions.endTime) {
		...
		var endDate = userOptions.endTime instanceof Date ? userOptions.endTime
				: parseRelativeDate(userOptions.endTime, options);
