...
/**
 * 
 * <p><b>标题：</b>auvtime\util$Lunar.</p>
 *
 * <p><b>描述：日期转换工具类</b></p>
 *
 * <p><b>版权：</b>Copyright (c) 2014 AUVTime</p>
 *
 * @author WangXianfeng<wangxianfeng@auvtime.com> 2014-9-21 下午4:28:10
 *
 * @since 1.0
 */
class Lunar {
    ...
    */
    ...
    /**
     * 根据阳历日期获取农历日期，阳历日期形如：1984-06-10 00:00:00
     * @param datetime $solarBirthday
     * @author WangXianfeng<wangxianfeng@auvtime.com> 2014-9-21 下午4:39:48
     */
    public function getLunarBirthdayFromSolar($solarBirthday){
        $year = substr($solarBirthday, 0,4);
        $month = substr($solarBirthday,5,2);
        $day = substr($solarBirthday,8,2);
        $time = substr($solarBirthday,11,8);
        Yii::info("@@@".$year.$month.$day.$time,'auvtime');
        $lunarDate = $this->convertSolarToLunar($year, $month, $day);
        $lunarBirthday = $lunarDate[0].$lunarDate[1].$lunarDate[2].$lunarDate[3].$time;
        Yii::info('@@@solarBirthday:'.$solarBirthday.',lunarBirthday:'.$lunarBirthday,'auvtime');
